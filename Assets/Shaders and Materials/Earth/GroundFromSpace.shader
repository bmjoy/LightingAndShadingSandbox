Shader "Atmosphere/GroundFromSpace"
{
	...
	SubShader
	{
    	...
    	Pass
    	{
			...
			uniform float fScaleHeight;		// The scale height (i.e. the altitude at which the atmosphere's average density is found)
			uniform float fScaleOverScaleDepth;	// fScale / fScaleHeight
			uniform float fHdrExposure;		// HDR exposure
			uniform float krPhaseBias;		// Tweaker for Rayleigh phase bias calculation (if applicable)
			...
			{
				float x = 1.0 - fCos;
				return fScaleHeight * exp(-0.00287 + x * (0.459 + x*(3.83 + x*(-6.80 + x * 5.25))));
			}
			...
			v2f vert(appdata_base v)
			{
				...
				fFar -= fNear;
				float fDepth = exp((fInnerRadius - fOuterRadius) / fScaleHeight);
